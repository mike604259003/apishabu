...
class Login extends CI_Controller{
    ...
    public function register(){
        ...
        if(isset($requestData['username']) && isset($requestData['password']) && isset($requestData['name']) && isset($requestData['position'])){
        $user = $requestData['username'];
        $pass = $requestData['password'];
       ...
       $decodepass=  sha1(md5($pass));
        ...
        if($result == true){
            $data = $this->user->searchuserlogin($user,$decodepass);
